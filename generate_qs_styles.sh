...
function generate_qs_bg_dark_style() {
    ...
    name_lc=`echo "$name" | tr '[:upper:]' '[:lower:]'`
    generate_overlay "theming_qs_bg_dark_$name_lc" "aicp.qs_bg_dark" "$my_path/qs_bg_dark_template" "$out_dir" "android" "$bg_dark_overlay_package.$name_lc" "" "$product_packages_makefile"
    style_file="$out_dir/res/values-night/colors.xml"
...
function generate_qs_bg_light_style() {
    ...
    name_lc=`echo "$name" | tr '[:upper:]' '[:lower:]'`
    generate_overlay "theming_qs_bg_light_$name_lc" "aicp.qs_bg_light" "$my_path/qs_bg_light_template" "$out_dir" "android" "$bg_dark_overlay_package.$name_lc" "" "$product_packages_makefile"
    style_file="$out_dir/res/values/colors.xml"
    sed -i "s|?background_floating|$background_floating|g" "$style_file"
    sed -i "s|?panel_background|$panel_background|g" "$style_file"
}
...
generate_qs_bg_dark_style "TransparentBlack" "#80000000" "#80303030"
generate_qs_bg_dark_style "TransparentGray" "#80303030" "#80424242"
