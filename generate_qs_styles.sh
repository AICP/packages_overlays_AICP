...
function generate_qs_bg_light_style() {
    ...
    name_lc=`echo "$name" | tr '[:upper:]' '[:lower:]'`
    generate_overlay "theming_qs_bg_light_$name_lc" "aicp.qs_bg_light" "$my_path/qs_bg_template" "$out_dir" "com.android.systemui" "$bg_dark_overlay_package.$name_lc" "SystemUI" "$product_packages_makefile"
